...
namespace GsalesPro
{
    public partial class InstanciaLocal : MetroFramework.Forms.MetroForm
    {
        public InstanciaLocal()
        {
            InitializeComponent();
        }
        private Dictionary<string, string> cidades()
        {
        ...
        private Dictionary<string, string> estatos()
        {
        ...
        private void InstanciaLocal_Load(object sender, EventArgs e)
        {
            selectEstado.DataSource = new BindingSource(estatos(), null);
            selectEstado.DisplayMember = "Value";
            selectEstado.ValueMember = "Key";

            selectCidade.DataSource = new BindingSource(cidades(), null);
            selectCidade.DisplayMember = "Key";
            selectCidade.ValueMember = "Value";
        }

        private void btn_save_local_ins_Click(object sender, EventArgs e)
        {
            try
            {
                string estado = ((KeyValuePair<string, string>)selectEstado.SelectedItem).Key;
                string base_operadora = ((KeyValuePair<string, string>)selectCidade.SelectedItem).Value;
                string cidade = ((KeyValuePair<string, string>)selectCidade.SelectedItem).Key;

                string[] baseConfig = base_operadora.Split('~');

                Config.BASE_OPERADORA = baseConfig[1].ToString();
                Config.COD_ESTADO_OPERADORA = baseConfig[0].ToString();
                Config.CIDADE_OPERADORA = cidade;
                Config.ESTADO_OPERADORA = estado;
